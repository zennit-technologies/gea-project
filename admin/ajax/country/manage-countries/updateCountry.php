...
if (isset($_POST) && isset($_SESSION['edit_country_id'])) {
    $id = $_POST['country_id'];
    $data = $_POST;
    ...
    if ($country_name != '' && strlen($country_name) >= 2 && strlen($country_name) <= 100 && preg_match($country_name_regx, $country_name) && $country_phone != '' && strlen($country_phone) >= 1 && strlen($country_phone) <= 5 && (preg_match($mobileRegx, $country_phone)) && $country_code != '' && strlen($country_code) == 2 && (preg_match($country_code_regx, $country_code)) && ($country_symbol == '' || (strlen($country_symbol) >= 1 && strlen($country_symbol) <= 10)) && ($country_capital == '' || (strlen($country_capital) >= 2 && strlen($country_capital) <= 100 && preg_match($country_name_regx, $country_capital))) && ($country_currency == '' || (strlen($country_currency) >= 1 && strlen($country_currency) <= 3 && preg_match($country_currency_regx, $country_currency))) && ($country_status == 1 || $country_status == 0)) {
        $sel_country = $conn->prepare("SELECT * FROM countries WHERE id=:id");
        $sel_country->execute([":id" => $id]);
        if ($sel_country->rowCount() >= 1) {
            $ins_qry = $conn->prepare("UPDATE countries SET phone=:phone, code=:code, name=:name, symbol=:symbol, capital=:capital, currency=:currency, active=:active, created_datetime=:created_datetime WHERE id=:id");
            $update_sql = $ins_qry->execute([":phone" => $country_phone, ":code" => $country_code, ":name" => $country_name, ":symbol" => $country_symbol, ":capital" => $country_capital, ":currency" => $country_currency, ":active" => $country_status, ":created_datetime" => $datetime, "id" => $id]);
            if ($update_sql) {
            ...
            echo json_encode(array("result" => 'country_not_exist'));
        }
    } else {
