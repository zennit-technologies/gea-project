...
$sel_qry = $conn->prepare("SELECT * FROM gea_product WHERE id=:id AND active=1");
$sel_qry->execute([":id" => $_SESSION['edit_gea_product_id']]);
...
<body class="main">
    ...
    <div class="content">
        <div class="top-bar">
            <!-- BEGIN: Breadcrumb -->
            <div class="-intro-x breadcrumb mr-auto hidden sm:flex"> <a href="">Dashboard</a> <i data-feather="chevron-right" class="breadcrumb__icon"></i> <a href="" class="breadcrumb--active">Edit GEA Product</div>
            <!-- END: Breadcrumb -->
                            ...
                            <form id="form_data">
                                <div id="" class="p-5 grid grid-cols-12 gap-4">
                                    <div class="mt-2 col-span-12 lg:col-span-6">
                                        ...
                                        <input id="name" name="name" type="text" class="form-control gea_name" placeholder="Enter Product Name (Ex: Green Energy)" value="<?php echo $fetch_data['name']; ?>">
                                    </div>
                                    <!-- For Description -->
                                    <div class="mt-2 col-span-12 lg:col-span-6">
                                        <label for="" class="form-label">1 GEA Value = <span id="gea_value_text"><?php echo $fetch_data['value'] . ' ' . $fetch_data['name']; ?></span> <span id="gea_value_text"></span> <span class="text-theme-6">*</span></label>
    ...
    <script>
        ...
        $("#form_data").on("submit", function(e) {
            ...
            if (val_regx.test(gea_value) && gea_value != '' && gea_value > 0 && name != '' && name.length >= 2 && name.length <= 100 && nameRegx.test(name) && (status == 1 || status == 0)) {
                ...
                form.append("gea_product_id", <?php echo $_SESSION['edit_gea_product_id'] ?>);
                $.ajax({
                    type: "POST",
                    url: "ajax/geaProduct/updateGeaProduct.php",
                    data: form,
                    dataType: "JSON",
                    processData: false,
                    contentType: false,
